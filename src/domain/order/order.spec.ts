...
describe('Order', () => {
	describe('placing an order', () => {
		...
		it('rejects an order with no items', () => {
			expect(() => Order.place('order-1', 'customer-1', [])).toThrow('Order must have at least one item');
		});
	...
	describe('OrderItem', () => {
		...
		it('rejects zero quantity', () => {
			expect(() => new OrderItem('product-1', 0)).toThrow('Order item quantity must be positive');
		});

		it('rejects negative quantity', () => {
			expect(() => new OrderItem('product-1', -1)).toThrow('Order item quantity must be positive');
		});
	});
});
